...
static void supernode2addr(n2n_edge_t * eee, char* addr) {
  ...
  if (strncmp(tmpAddr, "txt:", 4) == 0) {
        // 处理txt查询
        const char *domain = tmpAddr + 4; 
    ...
    if (strncmp(tmpAddr, "http:", 5) == 0 || strncmp(tmpAddr, "https:", 6) == 0) {
        char result[8192] = {0};
        CURL *curl;
        CURLcode res;
        char redirect_url[512] = {0};
	// 如果 addr 以 http: 或 https: 开头但没有 //
	if (strncmp(tmpAddr, "http:", 5) == 0 && strncmp(tmpAddr, "http://", 7) != 0) {
    		char fixed[512] = {0};
    		snprintf(fixed, sizeof(fixed), "http://%s", tmpAddr + 5); // 添加 //
    		safe_strncpy(tmpAddr, fixed, sizeof(tmpAddr));
	} else if (strncmp(tmpAddr, "https:", 6) == 0 && strncmp(tmpAddr, "https://", 8) != 0) {
    		char fixed[512] = {0};
